...
(function () {
  ...
  (function () {
    ...
    tools.copyInto({
      ...
      LIST: [
        '#000000', '#FFFF00', '#FFFFFF', '#1CE6FF', '#FF34FF', '#FF4A46', '#008941', '#006FA6', '#A30059',
      ...
      ]//Source: https://stackoverflow.com/questions/33295120/how-to-generate-gif-256-colors-palette/33295456#33295456
    }, spimosimUi.colorSet);
  }());

  function create1IntDrawFrame(colorSet, spinName, minValue) {
    var LIST = colorSet.LIST,
      n = LIST.length,
      INVALID = colorSet.INVALID;

    return function (pixels, protocol, t, offset) {
      function setPixel(value, i) {
        pixels[i] = LIST[(value - minValue) % n];
      };
  ...
  function create1WeightedSpinDrawFrame(colorSet, spinName, spinNumber) {
    ...
    var mUp = colorSet.CONT_UP[spinNumber],
      ...
      mUpComponents =
        [ mUp & 255, mUp & (255 << 8), mUp & (255 << 16), mUp & (255 << 24) ],
      mDownComponents = 
        [ mDown & 255, mDown & (255 << 8), mDown & (255 << 16), mDown & (255 << 24) ];
  ...
  modules.add('createDrawModes', 'spins and flips', function (config, colorSet) {
  ...
  modules.add('createDrawModes', 'int map', function (config, colorSet) {
    var drawModes = [],
      labelTexts = config.labelTexts || config.names;
    for (var i = 0, len = config.names.length; i < len; i++) {
      drawModes.push({
        draw: create1IntDrawFrame(colorSet, config.names[i], config.minValues[i]),
        text: labelTexts[i]
      });
    }
  ...
  modules.add('createDrawModes', 'grey scale spins', function (config, colorSet) {
    var drawModes = [];
  ...
  modules.add('createDrawModes', 'weighted spins', function (config, colorSet) {
    var drawModes = [];
    for (var i = 0, len = config.modes.length; i < len; i++) {
      var c = config.modes[i];
      drawModes.push({
        draw: create1WeightedSpinDrawFrame(colorSet, c.name, c.spinNumber || 0),
        text: c.labelText || c.name
      });
    }

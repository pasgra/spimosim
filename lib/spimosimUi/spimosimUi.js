...
self.spimosimUi = (function () {
  ...
  function initSettingsGui() {
    ...
    function processInitialStateFile(e) {
      ...
      reader.onload = function (e) {
        ...
        controls.restartOrUpdateSettings('settings', {'type': 'init-file'})
      };
    ...
    for (var name in this.modelConfig.controls.parameters) {
      if (this.modelConfig.controls.parameters.hasOwnProperty(name)) {
        ...
        let callback = (function(name) {
          return function (e) {
            controls.restartOrUpdateSettings('settings', {'type': 'setting', 'name': name});
          }
        })(name);
        this.attachEventListener({
   ...
   */
  Controls.prototype.restartOrUpdateSettings = function (action, trigger) {
    ...
    var modelSettings = this.parseModelSettings(false, trigger);
    ...
    switch (action) {
      case 'settings':
        ...
        if (wantedAction === 'always-restart' || (wantedAction !== 'do-nothing' && modelSettings.continuable === false)) {
          ...
          modelSettings = this.parseModelSettings(true, trigger);//Get settings again with option automaticStart
   ...
   */
  Controls.prototype.parseModelSettings = function (automaticStart, trigger) {
    var settings = {
      parameters: {},
      initialState: {},
      continuable: true,
...
//not .dygraph-title
  ...
  function PlotDisplay(plotter, settings) {
    ...
    var compactName = plotter.dataSource.compactName;
    if (plotter.dataSource.owner) {
      compactName = plotter.dataSource.owner.compactName;
    }
    this.compactName = compactName + '-' +
  ...
  PlotDisplay.prototype.updateFitSettings = function () {
    if (this.drawState.initialized) {
      ...
      if (len > 0) {
        ...
        this.updateSigFigAsNeeded();
      }
    }
  };

  PlotDisplay.prototype.updateSigFigAsNeeded = function () {
    // Dygraph does not set significant digits based on axes limits => fix this
    var digitsAfterDecimal = this.g.getOption("digitsAfterDecimal");
    var sigFigs = this.g.getOption("sigFigs") || this.g.getOption("digitsAfterDecimal") || 2;
    var yExtremes = this.g.yAxisExtremes()[0];
    var absMax = Math.max(Math.abs(yExtremes[0]) + Math.abs(yExtremes[1]));
    /*var extraWidthForScientificNotation = 0;
    if (absMax < 1e3 || absMax > 1e3) {
      extraWidthForScientificNotation = 3;
    } else if (absMax < 1e10 || absMax > 1e10) {
      extraWidthForScientificNotation = 4;
    } else if (absMax < 1e100 || absMax > 1e100) {
      extraWidthForScientificNotation = 5;
    }*/
    var sigFigsNeeded = Math.max(2, Math.min(17, Math.ceil(Math.log10(absMax) - Math.log10(Math.abs(yExtremes[0] - yExtremes[1])) + 2)));
    if (sigFigsNeeded > sigFigs || sigFigsNeeded < sigFigs - 2) {
      this.g.updateOptions({
        "sigFigs": sigFigsNeeded
      });
      var options = {
        "axes": {
          "y":{
            "axisLabelWidth": 8 * Math.max.apply(null, this.g.layout_.yticks.map(x=>x.label.length)) //(sigFigsNeeded + extraWidthForScientificNotation) * 9,
          }
        }
      };
      options["sigFigs"] = sigFigsNeeded;
      this.g.updateOptions(options);
      let area = this.g.layout_.getPlotArea();

      // reset canvas => necessary to remove artifacts from outside of dygraphs canvas clip
      for (var i = 0; i < 2; i++) {
        var ctx = [this.g.hidden_ctx_, this.g.canvas_ctx_][i];
  ...
  TimePlotDisplay.prototype.initGui = function () {
    ...
    this.attachEventListeners([
      ...
      },
      {
        ...
        dispatcher: this.plotter.initializer.clock
      }
    ]);

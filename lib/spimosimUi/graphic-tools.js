...
var graphicTools = (function() {
  ...
  function addVisibilityListener(element, parent) {
    parent.addEventListener('visibility', function (visible) {
  ...
  function createCodeSetting(config, settings) {
    var name = config.name,
      id = config.id || tools.randId(name),
      value = config.value,
      info = config.info,
    ...
    settingDiv.className = 'code-setting';
    ...
    for (var name in configs) {
      if (configs.hasOwnProperty(name)) {
        ...
        if (parent !== undefined) {
          ...
          addVisibilityListener(settings[name], settings[parent]);
        }
      }
    }
    for (var name in configs) {

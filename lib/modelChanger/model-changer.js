...
function ModelChanger(heading, modelIds, sections) {
  ...
  for (var i = 0, len = modelIds.length; i < len; i++) {
    this.modelConfigs.push(spimosimCore.modules.get('ModelConfig', modelIds[i]));
  }
  ...
  if (this.severalModels) {
    var modelName = graphicTools.getUriQuery('model');
    if (modelName) {
...
ModelChanger.prototype.initGUI = function (heading) {
  ...
  if (this.severalModels) {
    ...
    $(buttonPrevModel).on('click', function () {
      modelChanger.nextModel(true);
    });
    $(buttonNextModel).on('click', function () {
...
ModelChanger.prototype.changeModel = function (modelConfig) {
  ...
  if (this.severalModels) {
    graphicTools.setUriQuery('model', modelConfig.info.compactName);
  }

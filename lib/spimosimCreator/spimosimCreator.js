...
window.spimosimCreator = (function() {
  ...
  Creator.prototype.initGui = function (divConfig, divPreview) {
    ...
    this.showCodeWindows();
  };

  Creator.prototype.showCodeWindows = function () {
    this.domCache.configCode.changeSettingsFunction.codeMirror.setSize();
    this.domCache.configCode.stepFunction.codeMirror.setSize();
    this.domCache.configCode.modelCode.codeMirror.setSize();
  }
...
//      this.initializer.simulation.removeEventListener('error', this.initializer.simulation.errorLogger);
  ...
  Creator.prototype.updateStateVarCount = function () {
    ...
    for (var i = 0; i < min; i++) {
      this.domCache.divConfig.insertBefore(this.domCache.configStateVar[i].domElement, this.domCache.headingVideoConfig);
    }
    if (created < toShow) {
      for (var i = created; i < toShow; i++) {
        this.domCache.divConfig.insertBefore(this.createStateVarConfig(i), this.domCache.headingVideoConfig);
      }
    } else {
      for (var i = toShow; i < created; i++) {

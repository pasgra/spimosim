...
class Backend():
    def __init__(self, Model, handler, run_settings):
    ...
    def change_model_settings(self, model_settings, restart):
        try:
            ...
            if self.model == None or restart:
                self.model = self.Model(self, model_settings, self.run_settings)
            else:
    ...
    def send_requested_objects(self, names):
        ...
        msg = {"type": "requested objects", "objects": json.dumps(objects) }
        self.handler.write_message(msg)
...
class ChannelHandler(tornado.websocket.WebSocketHandler):
    def initialize(self, Model, run_settings):
        self.id = random.randint(0,1e6)

        try:
            self.backend = Backend(Model, self, run_settings)
        except Exception as e:
            traceback.print_exc()
...
def start(Model, run_settings):
    # Create tornado application and supply URL routes
    app = tornado.web.Application([ (run_settings.path, ChannelHandler, dict(Model=Model, run_settings=run_settings)) ])
    
    # Setup HTTP Server
    http_server = tornado.httpserver.HTTPServer(app)
    http_server.listen(run_settings.port, run_settings.address)
